...
set linesize 132
set pagesize 66
spool "C:\cprg250s\sis-project-5\build\sis_create_output.txt"
...
create table sis_credential(
	...
	type NVARCHAR2(2) not null 
		constraint sis_credential_type_ck check(type in ('MI', 'FT', 'CT', 'PD', 'AD', 'D'))
);
...
create table sis_student(
	...
	first_name NVARCHAR2(50) not null,
	last_name NVARCHAR2(50) not null,
	status NVARCHAR2(2) not null constraint sis_student_status_ck check(status in ('A', 'AP', 'S', 'E')),
	status_date date not null,
	phone nchar(12) constraint sis_student_phone_ck check (REGEXP_LIKE(phone, '^[0-9]{3}\.[0-9]{3}\.[0-9]{4}$')),
	email NVARCHAR2(100) not null 
		unique 
		constraint sis_student_email_ck
		check (regexp_like(email, '^[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,}$'))
);
...
add constraint sis_course_preq_pk primary key (course_code, pre_course_code);
...
create table sis_schedule_course(
	...
	semester_code nchar(6)
		constraint sis_schedule_course_semester_code_ck
		check(regexp_like(semester_code, '^(19|20)\d{2}(01|02|03|04)$')),
...
create table sis_student_credential(
	...
	GPA number(3,2) not null 
		constraint sis_student_credential_gpa_ck check(GPA between 0 and 4.00),
...
create table sis_student_course_record(
	...
	constraint sis_student_course_record_course_rgn_semester_fk 
		foreign key(course_registration#, semester_code) references sis_schedule_course(course_registration#, semester_code),
	constraint sis_student_course_record_student_id_fk 
		foreign key(student_id) references sis_student(student_id),
